...
import faiss
import imagehash
# `clip` is no longer directly used here, but managed by ClipManager
import torch
import numpy as np
...
class ReverseSearcher:
    ...
    def __init__(self):
        ...
        # +++ NEW: Centralized task management attributes +++
        self._lock = asyncio.Lock()
        self._reload_task = None
        self._reload_check_interval = 0  # Disabled by default
    ...
    async def configure_reloader(self, check_interval_seconds: int = 60):
        """
    ...
    async def stop_reloader_task(self):
    ...
    # +++ NEW: The background worker coroutine +++
    async def _periodic_reloader_task(self):
    ...
    # +++ NEW: The actual synchronous reload logic, safe to run in an executor +++
    def _internal_reload_logic(self):
        """
        ...
        try:
            ...
            new_faiss_index = faiss.read_index(INDEX_FILE)
            
            with open(ID_MAP_FILE, 'r') as f:
                new_faiss_to_db_id = json.load(f)
    ...
    # +++ MODIFIED +++: This method is now smarter and reloads only when the file changes.
    async def load_or_reload_indices(self):
        """
    ...
    # --- REFACTORED to be async and use db_utils ---
    async def _find_by_phash(self, p_hash_str: str) -> list:
        """Finds items in the database with an exact pHash match."""
        query = """
            SELECT 'image' as type, f.phash, ce.id as embedding_id
            FROM files f JOIN clip_embeddings ce ON f.id = ce.file_id
            WHERE f.phash = ?
            UNION ALL
    ...
    # --- REFACTORED to be async and use db_utils ---
    async def _find_by_precalculated_similarity(self, embedding_id: int, top_k: int):
        ...
        async with AsyncDBContext() as conn:
            related_items = await execute_db_query(conn, query, params, fetch_all=True)

        if not related_items:
            return []
            
        # aiosqlite.Row objects can be accessed by key
        related_embedding_ids = [item['other_id'] for item in related_items]
        details_map = await self._get_embedding_details(related_embedding_ids)
        ...
        for item in related_items:
            ...
            if other_id in details_map:
                details = details_map[other_id]
                details['similarity'] = f"{score:.4f}"
                results.append(details)
        return results

    async def _find_by_live_similarity(self, query_image: Image.Image, top_k: int):
        """
        ...
        image_input = clip_preprocess(query_image).unsqueeze(0).to(DEVICE)
        with torch.no_grad():
            query_features = clip_model.encode_image(image_input)
            query_features /= query_features.norm(dim=-1, keepdim=True)
        
        query_embedding = query_features.cpu().numpy().astype(np.float32)
            
        distances, faiss_indices = self.faiss_index.search(query_embedding, top_k)
        if not faiss_indices.size:
            return []

        db_ids = [self.faiss_to_db_id[i] for i in faiss_indices[0] if i < len(self.faiss_to_db_id)]
        details_map = await self._get_embedding_details(db_ids)
        
        results = []
        for i, db_id in enumerate(db_ids):
            if db_id in details_map:
                details = details_map[db_id]
                similarity_score = 1.0 - (distances[0][i]**2 / 2)
                details['similarity'] = f"{similarity_score:.4f}"
                results.append(details)
        return results

    # --- REFACTORED to be async and use db_utils ---
    # --- UPDATED for performance: Replaced correlated subquery with LEFT JOINs for tags ---
    async def _get_embedding_details(self, embedding_ids: list[int]) -> dict:
        """
        ...
        """
        async with AsyncDBContext() as conn:
            rows = await execute_db_query(conn, query, tuple(embedding_ids), fetch_all=True)
        ...
        for row in rows:
            ...
            card_data['id'] = card_data['file_id']
            results_map[card_data['embedding_id']] = card_data
        
        return results_map

    # --- UPDATED to await async db methods ---
    async def search(self, image_path: str, top_k: int = 5) -> dict:
        ...
        async with self._lock:
            ...
            query_phash = str(imagehash.phash(query_image))
            exact_matches = await self._find_by_phash(query_phash)
            
            # Stage 2 (Fast Path)
            if exact_matches:
                # Use key access for better readability with aiosqlite.Row
                embedding_id = exact_matches[0]['embedding_id']
                logger.info(f"Found exact match (embedding_id: {embedding_id}). Using pre-calculated similarities.")
                results = await self._find_by_precalculated_similarity(embedding_id, top_k)
                if results:
            ...
            results = await self._find_by_live_similarity(query_image, top_k)
...
async def main():
    ...
    try:
        ...
        searcher = await get_searcher() # Use singleton getter

        print(f"\nSearching for images similar to: {test_image_path}\n")
        search_results = await searcher.search(test_image_path, top_k=5)
